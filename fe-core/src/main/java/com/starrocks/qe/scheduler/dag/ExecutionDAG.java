...
public class ExecutionDAG {
    ...
    private final Map<PlanFragmentId, List<Long>> smartDispatchBackends = Maps.newHashMap();
    private final Map<PlanFragmentId, com.starrocks.qe.scheduler.dispatch.FragmentDispatchContext> dispatchContexts =
            Maps.newHashMap();
    private final Map<PlanFragmentId, com.starrocks.qe.scheduler.dispatch.FragmentInstanceStats> fragmentInstanceStats =
            Maps.newConcurrentMap();
    private com.starrocks.qe.scheduler.dispatch.DispatchTrainingRecorder dispatchTrainingRecorder = null;
    ...
    public void setSmartDispatchResult(DispatchResult dispatchResult) {
        if (dispatchResult == null) {
            return;
        }
        smartDispatchBackends.clear();
        smartDispatchBackends.putAll(dispatchResult.getFragmentToBackends());
    }

    public void setDispatchContexts(List<com.starrocks.qe.scheduler.dispatch.FragmentDispatchContext> contexts) {
        dispatchContexts.clear();
        if (contexts == null) {
            return;
        }
        for (com.starrocks.qe.scheduler.dispatch.FragmentDispatchContext context : contexts) {
            dispatchContexts.put(context.getFragmentId(), context);
        }
    }

    public Optional<com.starrocks.qe.scheduler.dispatch.FragmentDispatchContext> getDispatchContext(
            PlanFragmentId fragmentId) {
        return Optional.ofNullable(dispatchContexts.get(fragmentId));
    }

    /**
     * Get all recommended backends for a fragment.
     * Returns a list of backend IDs for multi-BE distribution.
     */
    public Optional<List<Long>> getSmartDispatchBackends(PlanFragmentId fragmentId) {
        return Optional.ofNullable(smartDispatchBackends.get(fragmentId));
    }

    /**
     * Get the primary (first) backend for a fragment.
     * Useful for gather fragments or backward compatibility.
     */
    public Optional<Long> getSmartDispatchBackend(PlanFragmentId fragmentId) {
        List<Long> backends = smartDispatchBackends.get(fragmentId);
        if (backends != null && !backends.isEmpty()) {
            return Optional.of(backends.get(0));
        }
        return Optional.empty();
    }

    public void setDispatchTrainingRecorder(com.starrocks.qe.scheduler.dispatch.DispatchTrainingRecorder recorder) {
        this.dispatchTrainingRecorder = recorder;
    }

    public Optional<com.starrocks.qe.scheduler.dispatch.DispatchTrainingRecorder> getDispatchTrainingRecorder() {
        return Optional.ofNullable(dispatchTrainingRecorder);
    }


    public com.starrocks.qe.scheduler.dispatch.FragmentInstanceStats getOrCreateFragmentInstanceStats(
            PlanFragmentId fragmentId, int expectedInstanceCount) {
        return fragmentInstanceStats.computeIfAbsent(fragmentId,
                k -> new com.starrocks.qe.scheduler.dispatch.FragmentInstanceStats(expectedInstanceCount));
    }
}
